...
// Static file serving for uploaded images (use /tmp in Vercel)
const isVercel = !!process.env.VERCEL;
const baseDir = isVercel ? '/tmp' : path.join(__dirname, '..');
const publicDir = path.join(baseDir, 'public');
const uploadDir = path.join(publicDir, 'profile-images');
...
const pool = new Pool({
  connectionString: `postgres://${process.env.DB_USER || process.env.VITE_DB_USER}:${process.env.DB_PASSWORD || process.env.VITE_DB_PASSWORD}@${process.env.DB_HOST || process.env.VITE_DB_HOST}:${process.env.DB_PORT || process.env.VITE_DB_PORT}/${process.env.DB_NAME || process.env.VITE_DB_NAME}?sslmode=require`,
  max: 20,
...
pool.on('error', (err) => {
  ...
  console.error('Host:', process.env.DB_HOST || process.env.VITE_DB_HOST);
  console.error('Database:', process.env.DB_NAME || process.env.VITE_DB_NAME);
});

// Health check query to verify connection
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.json({ 
      ...
      dbHost: process.env.DB_HOST || process.env.VITE_DB_HOST,
      dbName: process.env.DB_NAME || process.env.VITE_DB_NAME
    });
  } catch (error) {
    res.status(500).json({ 
      status: 'error', 
